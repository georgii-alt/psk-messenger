...
class ClickableImage(ButtonBehavior, KivyImage):
    ...
    def on_press(self):
        # При клике вызываем функцию открытия на весь экран
        self.callback(self.source)


# Обновленный пузырек сообщений с поддержкой кликабельных фото
class GlassBubble(MDBoxLayout):
    def __init__(self, text, is_my_msg=False, current_theme="Dark", is_media=False, media_type="image",
        ...
        if is_media:
            if media_type == "image":
                try:
                    ...
                    # Используем наш новый Кликабельный класс для фото
    ...
    def add_text_lbl(self, text, text_color):
        ...
        lbl.bind(texture_size=lambda instance, size: setattr(self, 'height', size[1] + 25))
        self.add_widget(lbl)
...
class ChatApp(MDApp):
    def build(self):
        ...
        self.nick_in = MDTextField(hint_text="Введите ваш никнейм...", size_hint_x=0.9, pos_hint={"center_x": 0.5},
                                   mode="rectangle")
        lay1.add_widget(self.nick_in)
        lay1.add_widget(MDRaisedButton(text="Войти в чат", pos_hint={"center_x": 0.5}, size_hint_x=0.9, height=50,
                                       md_bg_color=(0.25, 0.25, 0.28, 1), on_release=self.register_user))
        ...
        self.inp_lay = MDCard(orientation="horizontal", spacing=5, size_hint_y=None, height=55, padding=5,
                              md_bg_color=(0, 0, 0, 1))
        clip_btn = MDIconButton(icon="paperclip", theme_text_color="Custom", text_color=(0.7, 0.7, 0.75, 1),
        ...
        send_btn = MDRaisedButton(text="Отправить", size_hint_x=0.3, md_bg_color=(0.25, 0.25, 0.28, 1),
        ...
        # --- НОВЫЙ ЭКРАН ГАЛЕРЕИ (Полноэкранный просмотр изображений) ---
        self.scr_viewer = MDScreen(name="viewer")
        viewer_lay = MDBoxLayout(orientation="vertical", padding=10, spacing=10)

        # Кнопка возврата обратно в чат
        viewer_top = MDBoxLayout(orientation="horizontal", size_hint_y=None, height=50)
        viewer_top.add_widget(MDFlatButton(text="<- Назад в чат", theme_text_color="Custom", text_color=(1, 1, 1, 1),
                                           on_release=self.close_image_viewer))
        viewer_lay.add_widget(viewer_top)

        # Сам виджет полноэкранного фото
    ...
    def open_image_viewer(self, file_path):
        # Функция открывает экран галереи и подгружает туда нажатую картинку
        self.big_img.source = file_path
        self.sm.current = "viewer"

    def close_image_viewer(self, instance):
        # Возвращаемся из галереи обратно к переписке
                    ...
                    if res.status_code == 200 and res.json():
                        for m_id, m_data in res.json().items():
                            if m_id not in self.loaded_messages:
                                ...
                                # Передаем функцию обратного вызова self.open_image_viewer для обработки кликов по фото
                                Clock.schedule_once(lambda dt, t=display_text, m=is_mine, is_med=is_m,
                                                           mt=m_type: self.msg_box.add_widget(
                                    GlassBubble(text=t, is_my_msg=m, current_theme=self.theme_cls.theme_style,
                                                is_media=is_med, media_type=mt, click_callback=self.open_image_viewer)))
                except:
                    pass
            time.sleep(1)
